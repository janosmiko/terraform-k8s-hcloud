resource "null_resource" "ufw_master" {
  count = var.ufw_enabled ? 1 : 0
  ...
  provisioner "remote-exec" {
    inline = [
      "set -eux",
      "export MASTER_IPS='${join(" ", compact(concat(hcloud_server.master.*.ipv4_address, hcloud_server.secondary_masters.*.ipv4_address)))}'",
...
resource "null_resource" "ufw_secondary_master" {
  count = var.ufw_enabled ? length(hcloud_server.secondary_masters) : 0

  connection {
    host        = hcloud_server.secondary_masters[count.index].ipv4_address
    private_key = var.public_key == "" ? tls_private_key.this[0].private_key_pem : file(var.private_key)
  }
...
resource "null_resource" "ufw_node" {
